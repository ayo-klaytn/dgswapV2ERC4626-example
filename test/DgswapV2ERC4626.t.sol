...
import {IUniswapV2Factory} from "../src/erc4626/interfaces/IUniswapV2Factory.sol";
...
contract UniswapV2ERC4626Test is Test {
    ...
    IUniswapV2Router router = IUniswapV2Router(0x8203cBc504CE43c3Cad07Be0e057f25B1d4DB578);
    ...
    function test_DepositAndEarnFees() public {
        ...
        (uint256 usdtBefore, uint256 eldeBefore) = vault.getAssetsAmounts(depositAmount);
        ...
        (uint256 usdtAfter, uint256 eldeAfter) = vault.getAssetsAmounts(depositAmount);
    ...
    function _userDeposit(uint256 lpAmount) internal returns (uint256 shares) {
        (uint256 usdtNeeded, uint256 eldeNeeded) = vault.getAssetsAmounts(lpAmount);
    ...
    function _traderSwaps(uint256 numSwaps) internal {
        ...
        // Determine which is USDT and which is ELDE based on reserves
        // USDT has 6 decimals, ELDE has 18, so USDT reserve will be smaller
        bool isToken0USDT = reserve0 < reserve1;
        
        uint256 usdtReserve = isToken0USDT ? uint256(reserve0) : uint256(reserve1);
        uint256 eldeReserve = isToken0USDT ? uint256(reserve1) : uint256(reserve0);
        
        // 0.1% of pool per swap
        uint256 usdtSwapAmount = usdtReserve / 1000;
        uint256 eldeSwapAmount = eldeReserve / 1000;
        
        console2.log("  Swap amounts:");
        console2.log("    USDT:", usdtSwapAmount / 1e6);
        console2.log("    ELDE:", eldeSwapAmount / 1e18);
        ...
        for (uint256 i = 0; i < numSwaps; i++) {
            ...
            if (i % 2 == 0) {
                ...
                // Odd: Swap ELDE → USDT
                elde.approve(address(router), eldeSwapAmount);
        ...
        // Calculate fees from both directions
        uint256 usdtFeesGenerated = (usdtSwapAmount * (numSwaps / 2) * 3) / 1000;
        uint256 eldeFeesGenerated = (eldeSwapAmount * (numSwaps / 2) * 3) / 1000;
        
        console2.log("  Complete! Bidirectional trading:");
        console2.log("    USDT->ELDE swaps:", numSwaps / 2);
        console2.log("    ELDE->USDT swaps:", numSwaps / 2);
        console2.log("    USDT fees:", usdtFeesGenerated / 1e6);
        console2.log("    ELDE fees:", eldeFeesGenerated / 1e18);
    }
    
    function _calculateValue(uint256 usdtAmount, uint256 eldeAmount) internal view returns (uint256) {
        (uint112 reserve0, uint112 reserve1,) = pair.getReserves();
